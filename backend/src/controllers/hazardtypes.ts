...
const createHazardType = async (
  ...
  try {
    ...
    console.log(
      "Validation successful. Checking if a hazard type is created:",
      name
    );
...
const getAllHazardTypes = async (
  ...
  try {
    ...
    const name = value.name;
    console.log(
      "Validation successful. Checking to get all hazard types:",
      name
    );
    const hazardTypes = await HazardType.find().exec();

    return res.status(200).json({
      message: "All the Hazard Types",
      hazardTypes,
...
const getHazardTypeById = async (
  ...
  try {
    ...
    const name = value.name;
    console.log(
      "Validation successful. Checking to get one hazard type:",
      name
    );
    ...
    if (hazardType) {
      return res.status(200).json({
        message: "Your Specific Hazard Type",
        hazardType,
      });
    } else {
...
const updateHazardType = async (
  ...
  try {
    ...
    const name = value.name;
    console.log(
      "Validation successful. Checking to update a hazard type:",
      name
    );
    const hazardType = await HazardType.findById(hazardTypeId).exec();
...
const deleteHazardType = async (
  ...
  try {
    ...
    const name = value.name;
    console.log(
      "Validation successful. Checking to delete a hazard type:",
      name
    );
